...
def inputText(filename):
    ...
    text = ''.join(text)
    print('11111111111124225564655673111111111111111')
    return text
...
def DES_decrypt(filename,key,outputFile):
    ...
    for j in range(1,25001):
        for index in range(1,33):
            L,R = IP_Transposition(Ciphertext,index)
            # 生成Kset
            K = generateKset(key)
...
x_test = keras.preprocessing.sequence.pad_sequences(x_test,padding='post', maxlen=max_len)
print(x_train[0])
# 3DES加密
for i in range(1,25001):
    text = [hex(x) for x in x_train[i-1]]
    xlabel += text
    text=[]
    
xlabel = ','.join(map(str,xlabel))
   
with open('imdbplaintext.txt','w')as f:
    f.write(xlabel)
    f.close()
with open('imdbplaintext.txt','r')as f:
    xlabel = f.read()
xlabel = xlabel.split(',')
xlabel = [eval(x) for x in text]
#print('plaintext: ',[hex(x) for x in xlabel])
Cipher_forone = DES_encrypt('imdbplaintext.txt','key1.txt')

for i in range(len(Cipher_forone)):
        for j in range(len(Cipher_forone[i])):
            Cipher_forone[i][j] = int(Cipher_forone[i][j],16)
        print(i)

Cipher_forone = np.array(Cipher_forone)
...
x_test_encrypt = Cipher_forone[:10000]
...
model_encrypt.add(Embedding(max_features,
                    embedding_dims,
                    input_length=maxlen))
model_encrypt.add(Dropout(0.2))

# we add a Convolution1D, which will learn filters
# word group filters of size filter_length:
model_encrypt.add(Conv1D(filters,
                 kernel_size,
                 padding='valid',
                 activation='relu',
                 strides=1))
...
model_encrypt.add(Dense(hidden_dims))
model_encrypt.add(Dropout(0.2))
...
model_encrypt.compile(loss='binary_crossentropy',
              optimizer='adam',
              metrics=['accuracy'])
history_encrypt = model_encrypt.fit(Cipher_forone, y_train,
