...
testingDataFileHandler = open("mnist_test.csv","r")
testingDataList = testingDataFileHandler.readlines()
testingDataFileHandler.close()
...
xlabel_train = ','.join(map(str,xlabel_train))

with open('trainplaintext.txt','w')as f:
        f.write(xlabel_train)
        f.close()
with open('trainplaintext.txt','r')as f:
    xlabel_train = f.read()
    
xlabel_train = xlabel_train.split(',')
xlabel_train = [eval(x) for x in xlabel_train]
...
Cipher_train = Cipher_train.reshape(10000, 28, 28)
ylabel_train = np.array(ylabel_train)

x_test_encrypt = Cipher_train[:2000]
y_test_encrypt = ylabel_train[:2000]

model_encrypt = Sequential()
# model_encrypt.add(Embedding(10000, 120))
model_encrypt.add(LSTM(120, input_shape=(28, 28)))
model_encrypt.add(Dense(16))
model_encrypt.add(Activation('relu'))
model_encrypt.add(Dense(1))
model_encrypt.add(Activation('sigmoid'))
# model_encrypt.add(Dense(10, activation='softmax'))
model_encrypt.compile(loss='mae',
                      optimizer='rmsprop',
                      metrics=['accuracy'])
history_encrypt = model_encrypt.fit(Cipher_train, ylabel_train,                                    
