...
if __name__ == "__main__":
    ...
    ylabel = []
    for i in range(1,10001):
        text = trainingDataList[i-1].split(',')
        text = [eval(x) for x in text]
        ylabel.append(text[0])
        text.pop(0)
        text = [hex(x) for x in text]
        xlabel += text
        text=[]
        print(i)
    ...
    Cipher_train = DES_encrypt('trainplaintext.txt','key1.txt')

    print('2222222222222222222222222222222222222222')
          Cipher_test[i][j] = int(Cipher_test[i][j],16)


for i in range(len(Cipher_train)):
    for j in range(len(Cipher_train[i])):
        
Cipher_train[i][j] = int(Cipher_train[i][j],16)

img_rows = 28
img_cols = 28

Cipher_train = np.array(Cipher_train)     
ylabel = np.array(ylabel)
    # 1. 데이터셋 준비하기
    # 훈련셋과 검증셋 분리
x_train = Cipher_train[:6000]
y_train = ylabel[:6000]
    
x_val = Cipher_train[6000:8000]
y_val = ylabel[6000:8000]
    
x_test = Cipher_train[8000:]
y_test = ylabel[8000:]
...
model_encrypt.add(Conv2D(32, kernel_size=(5, 5), strides=(1, 1), padding='same',
                 activation='relu',
                 input_shape=input_shape))
model_encrypt.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
model_encrypt.add(Conv2D(64, (2, 2), activation='relu', padding='same'))
model_encrypt.add(MaxPooling2D(pool_size=(2, 2)))
model_encrypt.add(Dropout(0.25))
model_encrypt.add(Flatten())
model_encrypt.add(Dense(1000, activation='relu'))
model_encrypt.add(Dropout(0.5))
model_encrypt.add(Dense(num_classes, activation='softmax'))
model_encrypt.summary()

model_encrypt.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
history_encrypt = model_encrypt.fit(x_train, y_train,batch_size=batch_size,epochs=epochs,verbose=1,validation_data=(x_test, y_test))
