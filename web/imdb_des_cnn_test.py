...
model = load_model('imdb_des_cnn.h5')
...
max_len = 256
xhat = sequence.pad_sequences(xhat, maxlen=max_len)
...
xhat_encrypt = DES_encrypt('imdbplaintext.txt','key1.txt')

for i in range(len(xhat_encrypt)):
        for j in range(len(xhat_encrypt[i])):
            xhat_encrypt[i][j] = int(xhat_encrypt[i][j],16)

xhat_encrypt = np.array(xhat_encrypt)


yhat = model.predict_classes(xhat_encrypt)
